...
- 🗣️ **多供应商统一架构**：新增供应商只需写一个薄壳子类（10 行）+ 一个前端包装层（30 行）
- 🌍 **多语种**：16 种语言支持（zh / en / fr / de / ja / ko / ru / pt / th / id / vi / es / it / ms / fil / ar）
- 🎨 **可视化音色管理**：Dashboard 内置音色创建（上传 / 公网 URL / 声音设计）、试听、列表、删除
- 🔁 **多供应商回退**：支持配置多个供应商，按 `priority` 自动降级
- 🛡️ **参数校验 + 兜底清理**：`validate_params` 失败时自动 `sanitize_params` 兜底，避免合成炸掉
